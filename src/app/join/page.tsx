...
// Types to handle our realtime data
...
export default function JoinPage() {
    ...
    // REALTIME LISTENER: Watch for factions being created or members joining
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                ...
                async () => {
                    // Refetch faction data when there's an update (new faction created or named)
            ...
            .on(
                ...
                async () => {
                    // Refetch to update member counts on the hero cards
                    const status = await checkFactionStatus(sessionId);
                    if (status.factions) setFactions(status.factions);
                }
            )
    ...
    const handleNicknameSubmit = async (e: React.FormEvent) => {
        ...
        // 1. Base Join: Registers the participant, but doesn't assign a team yet
        ...
        if (status.factions.length < status.maxFactions) {
            ...
            setRole("MEMBER");
            setStep(4); // Go to Faction Picker / Waiting Room
        }
    ...
    // Calculate if we are waiting for leaders to finish naming
                ...
                {step === 4 && role === 'MEMBER' && (
                    <div className="flex flex-col gap-6 animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {/* SKELETON CARDS (Show while waiting for leaders to finish typing) */}
