...
export default async function DashboardPage({ searchParams }: DashboardProps) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <main className="flex-1 pt-24 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto w-full pb-12">
                ...
                {currentTab === "quizzes" && (
                    quizzes.length === 0 ? (
                        <div className="bg-white border border-slate-200 rounded-xl p-8 sm:p-12 text-center flex flex-col items-center">
                            ...
                            <p className="text-slate-500 mt-2 mb-6 max-w-md text-sm sm:text-base">You haven't generated any assessments. Head over to the generator to turn your study materials into interactive quizzes.</p>
                            <Link href="/" className="bg-slate-900 text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-slate-800 transition-colors w-full sm:w-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                            {quizzes.map((quiz) => (
                                <Link
                                    href={`/quiz/${quiz.id}`}
                                    key={quiz.id}
                                    className="group bg-white border border-slate-200 rounded-xl p-5 sm:p-6 hover:shadow-md hover:border-[#4ce0a3] transition-all duration-300 flex flex-col"
                                >
